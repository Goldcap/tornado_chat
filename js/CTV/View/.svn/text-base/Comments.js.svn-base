...
CTV.View.Comments = function(options) {
	this.options = $.extend({
		list: null,
		filmId: 0,
		screeningId: 0,		
...
CTV.View.Comments.prototype = {
	...
	<div class="comment-item">\
		...
		<div class="comment-item-content">\
			<div class="comment-item-details">\
				<a href="/profile/{{authorid}}" class="comment-item-user">{{authorname}}</a>\
	...
	init: function() {
		...
		if ( !! $('#comment-box').length) {
			this.CommentBox = new CTV.View.CommentBox({
		...
		this.ReplyBox = new CTV.View.CommentBox({
			isReply: true,
			filmId: this.options.filmId,
			screeningId: this.options.screeningId,
			userId: this.options.userId
		...
		if ( !! $('#pagination').length) {
			this.pagination = new CTV.View.Pagination({}, '#pagination');
	...
	buildFacebookShareLink: function(data) {
		...
		string += '&description=' + encodeURIComponent(data.authorname + ' wrote: ' + data.comment + ' ' + this.options.fbShareDescription);
		string += '&redirect_uri=' + encodeURIComponent(window.location.href)
		return string;
	},
	buildTwitterShareLink: function(data) {
		return 'http://twitter.com/share?text=' + encodeURIComponent(data.authorname + ' wrote "' + data.comment + '"' + this.options.tShareCaption + ' ') + '&url=' + encodeURIComponent(window.location.href)
	},
...
CTV.View.CommentBox = function(options, domNode) {
	this.options = $.extend({
		urls: {
...
CTV.View.Pagination = function(options, domNode) {
	this.options = $.extend({
		list: null,
		urls: {
			post: '/services/conversation/post'
...
CTV.View.Pagination.prototype = {
	templatePage: '<li><span class="button button-medium button-black button-page">{{page}}</span></li>',
	init: function() {
	...
	attachEvents: function() {
		$('.button-page', this.domNode).live('click', _.bind(this.onButtonClick, this))
	},
